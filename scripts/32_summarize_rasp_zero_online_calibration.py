...
MANIFEST = ROOT / "configs" / "generated_rasp_zero_online_calibration" / "manifest.json"
OUTPUT = ROOT / "runs" / "rasp_zero_online_calibration" / "summary.csv"
...
def main() -> None:
    ...
    for row in rows:
        print(
            f"{row['dataset']:7s} {row['policy']:14s} "
