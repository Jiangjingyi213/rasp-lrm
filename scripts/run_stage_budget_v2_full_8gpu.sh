...
CONFIG="${CONFIG:-configs/generated_stage_budget_output_aware_v1/budget_v2_full.template.yaml}"
DEV_SELECTION="${RUN_ROOT}/07_budget_v2_grid_repair/01_dev_seed3/budget_v2_dev_selection.json"
SEED42_SELECTION="${RUN_ROOT}/07_budget_v2_grid_repair/02_seed42_confirmation/budget_v2_seed42_selection.json"
...
if [[ -z "${selected_method}" ]]; then
  echo "Budget v2 dev did not pass; stopping before full."
  exit 0
fi
...
if [[ "${seed42_passed}" != "1" ]]; then
  echo "Budget v2 seed42 confirmation did not pass; stopping before full."
  exit 0
fi
...
for seed in ${STAGE_BUDGET_SEEDS:-3}; do
  phase_root="${RUN_ROOT}/07_budget_v2_grid_repair/03_frozen_full/seed_${seed}"
  CONFIG="${CONFIG}" RUN_ROOT="${phase_root}" SOURCE_ROOT="${SOURCE_ROOT}" LOG_DIR="${LOG_ROOT}/07_budget_v2_grid_repair/03_frozen_full/seed_${seed}" \
