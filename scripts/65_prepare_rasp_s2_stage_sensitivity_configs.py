...
def main() -> None:
    ...
    for source, template in SOURCES:
        ...
        for shard, offset in enumerate(range(0, total, shard_size)):
            ...
            cfg["paths"] = {
                ...
                "counterfactuals": f"{run_dir}/03_stage_window_counterfactuals.jsonl",
                "probe_dataset": f"{run_dir}/05_stage_probe_dataset.jsonl",
                "probe_hidden_states": f"{run_dir}/05_stage_probe_hidden_states.pt",
                "aligned_window_bank_summary": f"{run_dir}/06_stage_window_bank_summary.json",
                "aligned_window_bank_validation": f"{run_dir}/07_stage_window_bank_validation.json",
            }
    ...
    print(f"generated {len(manifest)} S2 smoke shards across {gpu_count} GPUs")
