...
SEED42_SELECTION="${RUN_ROOT}/10_budget_v41_stage_lift_32p/02_seed42_confirmation/budget_v41_seed42_selection.json"
...
if [[ -z "${selected_method}" ]]; then
  echo "Budget v4.1 dev did not pass; stopping before full."
  exit 0
fi
...
for seed in ${STAGE_BUDGET_SEEDS:-3}; do
  phase_root="${RUN_ROOT}/10_budget_v41_stage_lift_32p/03_frozen_full/seed_${seed}"
  CONFIG="${CONFIG}" RUN_ROOT="${phase_root}" SOURCE_ROOT="${SOURCE_ROOT}" LOG_DIR="${LOG_ROOT}/10_budget_v41_stage_lift_32p/03_frozen_full/seed_${seed}" \
