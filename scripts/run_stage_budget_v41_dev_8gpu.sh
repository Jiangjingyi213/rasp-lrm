...
CONFIG="${CONFIG:-configs/generated_stage_budget_output_aware_v1/budget_v41_dev.yaml}"
...
REFERENCE_METHOD="dynamic_global_activation_budgeted_v4_mild_32p"
...
if [[ -z "${selected_method}" ]]; then
  echo "Budget v4.1 smoke did not pass; stopping before dev."
  exit 0
fi
...
for seed in ${SEEDS}; do
  phase_root="${RUN_ROOT}/10_budget_v41_stage_lift_32p/01_dev_seed3/seed_${seed}"
  CONFIG="${CONFIG}" RUN_ROOT="${phase_root}" SOURCE_ROOT="${SOURCE_ROOT}" LOG_DIR="${LOG_ROOT}/10_budget_v41_stage_lift_32p/01_dev_seed3/seed_${seed}" \
    STAGE_SEED="${seed}" STAGE_FINAL_SEEDS="${seed}" DATASETS_OVERRIDE="gsm8k math500" STAGE_FINAL_EVAL_LIMIT="${STAGE_BUDGET_V41_DEV_LIMIT:-520}" STAGE_FINAL_METHODS="${selected_method},${REFERENCE_METHOD}" \
...
"${PYTHON_BIN}" -m src.main_select_stage_budget_output_aware \
  ...
  --target-max 0.340 \
  --target-center 0.330 \
  --phase-root "${RUN_ROOT}/10_budget_v41_stage_lift_32p/01_dev_seed3" \
  ...
  --output "${RUN_ROOT}/10_budget_v41_stage_lift_32p/01_dev_seed3/budget_v41_dev_selection.json"
