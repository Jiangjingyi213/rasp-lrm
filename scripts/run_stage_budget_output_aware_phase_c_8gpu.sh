...
CONFIG="${CONFIG:-configs/generated_stage_budget_output_aware_v1/combined_dev.yaml}"
METHODS="dynamic_global_output_aware_budgeted"
SEEDS="${STAGE_BUDGET_SEEDS:-3}"

bash scripts/prepare_stage_budget_output_aware_v1.sh

for seed in ${SEEDS}; do
  phase_root="${RUN_ROOT}/03_combined_dev/seed_${seed}"
  CONFIG="${CONFIG}" RUN_ROOT="${phase_root}" SOURCE_ROOT="${SOURCE_ROOT}" LOG_DIR="${LOG_ROOT}/03_combined_dev/seed_${seed}" \
...
"${PYTHON_BIN}" -m src.main_select_stage_budget_output_aware \
  --phase c \
  --phase-root "${RUN_ROOT}/03_combined_dev" \
  --reference-root "${RUN_ROOT}/01_budget_only_dev" \
  --output "${RUN_ROOT}/03_combined_dev/phase_c_selection.json"
