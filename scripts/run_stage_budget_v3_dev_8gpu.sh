...
CONFIG="${CONFIG:-configs/generated_stage_budget_output_aware_v1/budget_v3_dev.yaml}"
SEEDS="${STAGE_BUDGET_SEEDS:-3}"
SMOKE_SELECTION="${RUN_ROOT}/08_budget_v3_actual_calibrated/00_smoke_actual_pruning/budget_v3_smoke_selection.json"
...
if [[ -z "${selected_method}" ]]; then
  echo "Budget v3 smoke did not pass; stopping before dev."
  exit 0
fi
...
for seed in ${SEEDS}; do
  phase_root="${RUN_ROOT}/08_budget_v3_actual_calibrated/01_dev_seed3/seed_${seed}"
  CONFIG="${CONFIG}" RUN_ROOT="${phase_root}" SOURCE_ROOT="${SOURCE_ROOT}" LOG_DIR="${LOG_ROOT}/08_budget_v3_actual_calibrated/01_dev_seed3/seed_${seed}" \
    STAGE_SEED="${seed}" STAGE_FINAL_SEEDS="${seed}" DATASETS_OVERRIDE="gsm8k math500" STAGE_FINAL_EVAL_LIMIT="${STAGE_BUDGET_V3_DEV_LIMIT:-520}" STAGE_FINAL_METHODS="${selected_method}" \
...
"${PYTHON_BIN}" -m src.main_select_stage_budget_output_aware \
  ...
  --selection-mode dev \
  --phase-root "${RUN_ROOT}/08_budget_v3_actual_calibrated/01_dev_seed3" \
  ...
  --output "${RUN_ROOT}/08_budget_v3_actual_calibrated/01_dev_seed3/budget_v3_dev_selection.json"
