...
CONFIG_PATH="${CONFIG:-configs/stage_calibrated_pruning/mixed_reasoning_seed3_safe_dynamic_v2_target_pruning_downstream.yaml}"
PROFILE="${PROFILE:-pilot}"
SOURCE_ROOT="${SOURCE_ROOT:-runs/08_stage_calibrated_pruning/main_pilot_mixed_reasoning_seed3}"
RUN_ROOT="${RUN_ROOT:-runs/08_stage_calibrated_pruning/main_pilot_mixed_reasoning_seed3_safe_dynamic_v2_target_pruning_downstream}"
FINAL_METHODS="${STAGE_FINAL_METHODS:-structured_dense,static_t20,t20_uniform,t20_v24_scaled,t20_math_safe,static_t30,t30_uniform,t30_v24_scaled,t30_math_safe,static_t60,t60_prior_heavy,t60_uniform_prior_heavy}"
...
echo "START V2 target-pruning sharded final; methods=${FINAL_METHODS}; shards=${SHARD_COUNT}; global_limit=${FINAL_EVAL_LIMIT}"
pids=()
for shard_index in $(seq 0 $((SHARD_COUNT - 1))); do
  gpu="${GPUS[$shard_index]}"
  log_path="${LOG_DIR}/safe_dynamic_v2_target_pruning_seed${SEED}_shard${shard_index}_of${SHARD_COUNT}_gpu${gpu}.log"
...
if [[ "${failed}" -ne 0 ]]; then
  echo "At least one V2 target-pruning shard failed. Check logs under ${LOG_DIR}." >&2
  exit 1
fi
