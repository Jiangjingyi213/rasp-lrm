...
CONFIG_PATH="${CONFIG:-configs/stage_calibrated_pruning/mixed_reasoning_seed3_static_core_residual_v4_3_final_light.yaml}"
PROFILE="${PROFILE:-pilot}"
SOURCE_ROOT="${SOURCE_ROOT:-runs/08_stage_calibrated_pruning/main_pilot_mixed_reasoning_seed3_static_core_residual_v4_2}"
RUN_ROOT="${RUN_ROOT:-runs/08_stage_calibrated_pruning/main_pilot_mixed_reasoning_seed3_static_core_residual_v4_3_final_light}"
FINAL_EVAL_LIMIT="${STAGE_FINAL_EVAL_LIMIT:--1}"
FINAL_METHODS="${STAGE_FINAL_METHODS:-structured_dense,static_core_residual_v4_3_final_light,static_matched_global}"
...
echo "START evaluate_dev sanity for V4.3 on GPU ${DEV_GPU}"
...
"${PYTHON_BIN}" -m src.main_stage_calibrated_pruning \
  ...
  --stage evaluate_dev \
  --force \
  > "${LOG_DIR}/v4_3_final_light_evaluate_dev.log" 2>&1
...
for shard_index in $(seq 0 $((SHARD_COUNT - 1))); do
  ...
  log_path="${LOG_DIR}/v4_3_final_light_shard${shard_index}_of${SHARD_COUNT}_gpu${gpu}.log"
