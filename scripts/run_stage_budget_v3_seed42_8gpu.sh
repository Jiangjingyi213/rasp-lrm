...
CONFIG="${CONFIG:-configs/generated_stage_budget_output_aware_v1/budget_v3_seed42.yaml}"
DEV_SELECTION="${RUN_ROOT}/08_budget_v3_actual_calibrated/01_dev_seed3/budget_v3_dev_selection.json"
...
phase_root="${RUN_ROOT}/08_budget_v3_actual_calibrated/02_seed42_confirmation/seed_42"
CONFIG="${CONFIG}" RUN_ROOT="${phase_root}" SOURCE_ROOT="${SOURCE_ROOT}" LOG_DIR="${LOG_ROOT}/08_budget_v3_actual_calibrated/02_seed42_confirmation/seed_42" \
  STAGE_SEED=42 STAGE_FINAL_SEEDS=42 DATASETS_OVERRIDE="gsm8k math500" STAGE_FINAL_EVAL_LIMIT="${STAGE_BUDGET_V3_DEV_LIMIT:-520}" STAGE_FINAL_METHODS="${selected_method}" \
...
"${PYTHON_BIN}" -m src.main_select_stage_budget_output_aware \
  ...
  --seed 42 \
  --phase-root "${RUN_ROOT}/08_budget_v3_actual_calibrated/02_seed42_confirmation" \
  ...
  --output "${RUN_ROOT}/08_budget_v3_actual_calibrated/02_seed42_confirmation/budget_v3_seed42_selection.json"
