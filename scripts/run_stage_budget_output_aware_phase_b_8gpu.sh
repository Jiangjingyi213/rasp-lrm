...
CONFIG="${CONFIG:-configs/generated_stage_budget_output_aware_v1/output_aware_only_dev.yaml}"
METHODS="dynamic_global_output_aware_fixed_t30,current_t30_stage_output_aware"
SEEDS="${STAGE_BUDGET_SEEDS:-3}"

bash scripts/prepare_stage_budget_output_aware_v1.sh

for seed in ${SEEDS}; do
  phase_root="${RUN_ROOT}/02_output_aware_only_dev/seed_${seed}"
  CONFIG="${CONFIG}" RUN_ROOT="${phase_root}" SOURCE_ROOT="${SOURCE_ROOT}" LOG_DIR="${LOG_ROOT}/02_output_aware_only_dev/seed_${seed}" \
...
"${PYTHON_BIN}" -m src.main_select_stage_budget_output_aware \
  --phase b \
  --phase-root "${RUN_ROOT}/02_output_aware_only_dev" \
  --reference-root "${RUN_ROOT}/01_budget_only_dev" \
  --output "${RUN_ROOT}/02_output_aware_only_dev/phase_b_selection.json"
