...
CONFIG_PATH="${CONFIG:-configs/stage_calibrated_pruning/mixed_reasoning_seed3_safe_dynamic_v2_3_v2_4_downstream.yaml}"
PROFILE="${PROFILE:-pilot}"
SOURCE_ROOT="${SOURCE_ROOT:-runs/08_stage_calibrated_pruning/main_pilot_mixed_reasoning_seed3}"
RUN_ROOT="${RUN_ROOT:-runs/08_stage_calibrated_pruning/main_pilot_mixed_reasoning_seed3_safe_dynamic_v2_3_v2_4_downstream}"
FINAL_METHODS="${STAGE_FINAL_METHODS:-structured_dense,static_matched_global,safe_dynamic_v2_current,safe_dynamic_v2_2_gsm_strong,safe_dynamic_v2_3_hybrid_stable,safe_dynamic_v2_4_final_prune}"
...
echo "START V2.3/V2.4 sharded final; methods=${FINAL_METHODS}; shards=${SHARD_COUNT}; global_limit=${FINAL_EVAL_LIMIT}"
pids=()
for shard_index in $(seq 0 $((SHARD_COUNT - 1))); do
  gpu="${GPUS[$shard_index]}"
  log_path="${LOG_DIR}/safe_dynamic_v2_3_v2_4_seed${SEED}_shard${shard_index}_of${SHARD_COUNT}_gpu${gpu}.log"
...
if [[ "${failed}" -ne 0 ]]; then
  echo "At least one V2.3/V2.4 shard failed. Check logs under ${LOG_DIR}." >&2
  exit 1
fi
