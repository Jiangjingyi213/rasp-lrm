...
write_config() {
  ...
  if [[ "${controller}" == "fixed" ]]; then
    run_dir="runs/eval_rasp_train_v2_1_dense_${dataset}_smoke"
    config="${CONFIG_DIR}/dense_${dataset}.yaml"
    controller_config=$'  controller: fixed\n  fixed_ratio: 0.00'
  else
    run_dir="runs/eval_rasp_train_v2_1_${tag}_${dataset}_smoke"
...
for tag_budget in "b15 0.15" "b20 0.20"; do
  ...
  for dataset in gsm8k math500; do
    ...
    "${PYTHON}" -m src.main_compare_rasp_train_online \
      --dense "runs/eval_rasp_train_v2_1_dense_${dataset}_smoke/01_trajectories.jsonl" \
      --policy "runs/eval_rasp_train_v2_1_${tag}_${dataset}_smoke/01_trajectories.jsonl" \
      --output "runs/eval_rasp_train_v2_1_${tag}_${dataset}_smoke/14_paired_dense_comparison.json"
  done
done
