...
CONFIG_DIR="${CONFIG_DIR:-/tmp/rasp_train_v2_1_online_smoke_configs}"
mkdir -p "${CONFIG_DIR}"
...
write_config() {
  ...
  local limit="$4"
  local controller="$5"
  local policy="${OUTPUT_ROOT}/shared/rasp_train_policy.pt"
  local run_dir
  local config
  local controller_config
  if [[ "${controller}" == "fixed" ]]; then
    run_dir="runs/04_rasp_train/01_legacy/eval_rasp_train_v2_1_dense_${dataset}_smoke"
    config="${CONFIG_DIR}/dense_${dataset}.yaml"
    controller_config=$'  controller: fixed\n  fixed_ratio: 0.00'
  else
    run_dir="runs/04_rasp_train/01_legacy/eval_rasp_train_v2_1_${tag}_${dataset}_smoke"
...
for tag_budget in "b15 0.15" "b20 0.20"; do
  ...
  for dataset in gsm8k math500; do
    ...
    "${PYTHON}" -m src.main_compare_rasp_train_online \
      --dense "runs/04_rasp_train/01_legacy/eval_rasp_train_v2_1_dense_${dataset}_smoke/01_trajectories.jsonl" \
      --policy "runs/04_rasp_train/01_legacy/eval_rasp_train_v2_1_${tag}_${dataset}_smoke/01_trajectories.jsonl" \
      --output "runs/04_rasp_train/01_legacy/eval_rasp_train_v2_1_${tag}_${dataset}_smoke/14_paired_dense_comparison.json"
  done
done
