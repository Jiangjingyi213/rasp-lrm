...
CONFIG_PATH="${CONFIG:-configs/stage_calibrated_pruning/llama31_tulu_8b_rasp_t20_full.yaml}"
...
run_generate_shards() {
  ...
  for shard_index in $(seq 0 $((GENERATE_SHARD_COUNT - 1))); do
    ...
    local log_path="${LOG_DIR}/llama31_tulu_rasp_t20_${profile}_generate_shard${shard_index}_of${GENERATE_SHARD_COUNT}_gpu${gpu}.log"
...
run_final_shards() {
  ...
  for shard_index in $(seq 0 $((FINAL_SHARD_COUNT - 1))); do
    ...
    local log_path="${LOG_DIR}/llama31_tulu_rasp_t20_${profile}_final_shard${shard_index}_of${FINAL_SHARD_COUNT}_gpu${gpu}.log"
...
if [[ "${RUN_PRIOR_FIRST}" == "1" ]]; then
  echo "START Llama-3.1-Tulu-3-8B RASP t20 prior/smoke"
  run_profile "${PRIOR_PROFILE}" "${PRIOR_EVAL_LIMIT:-16}"
  echo "DONE Llama-3.1-Tulu-3-8B RASP t20 prior/smoke"
fi

if [[ "${RUN_FULL_AFTER_PRIOR}" == "1" ]]; then
  echo "START Llama-3.1-Tulu-3-8B RASP t20 pilot/full"
  run_profile "${FULL_PROFILE}" "${FULL_EVAL_LIMIT:--1}"
  echo "DONE Llama-3.1-Tulu-3-8B RASP t20 pilot/full"
fi
