...
CONFIG="${CONFIG:-configs/generated_stage_budget_output_aware_v1/budget_v41_seed42.yaml}"
...
phase_root="${RUN_ROOT}/10_budget_v41_stage_lift_32p/02_seed42_confirmation/seed_42"
CONFIG="${CONFIG}" RUN_ROOT="${phase_root}" SOURCE_ROOT="${SOURCE_ROOT}" LOG_DIR="${LOG_ROOT}/10_budget_v41_stage_lift_32p/02_seed42_confirmation/seed_42" \
  STAGE_SEED=42 STAGE_FINAL_SEEDS=42 DATASETS_OVERRIDE="gsm8k math500" STAGE_FINAL_EVAL_LIMIT="${STAGE_BUDGET_V41_DEV_LIMIT:-520}" STAGE_FINAL_METHODS="${selected_method},${REFERENCE_METHOD},${BASELINE_METHOD}" \
...
"${PYTHON_BIN}" -m src.main_select_stage_budget_output_aware \
  ...
  --target-max 0.340 \
  --target-center 0.330 \
  --phase-root "${RUN_ROOT}/10_budget_v41_stage_lift_32p/02_seed42_confirmation" \
  ...
  --output "${RUN_ROOT}/10_budget_v41_stage_lift_32p/02_seed42_confirmation/budget_v41_seed42_selection.json"
