...
CONFIG_PATH="${CONFIG:-configs/stage_calibrated_pruning/mixed_reasoning_seed3_strict_static_matched_downstream.yaml}"
PROFILE="${PROFILE:-pilot}"
SOURCE_ROOT="${SOURCE_ROOT:-runs/08_stage_calibrated_pruning/main_pilot_mixed_reasoning_seed3}"
RUN_ROOT="${RUN_ROOT:-runs/08_stage_calibrated_pruning/main_pilot_mixed_reasoning_seed3_strict_static_matched_downstream}"
FINAL_METHODS="${STAGE_FINAL_METHODS:-structured_dense,static_t20_0p25,static_t30_0p36,static_t30_0p37}"
...
echo "START strict static matched sharded final; methods=${FINAL_METHODS}; shards=${SHARD_COUNT}; global_limit=${FINAL_EVAL_LIMIT}"
pids=()
for shard_index in $(seq 0 $((SHARD_COUNT - 1))); do
  gpu="${GPUS[$shard_index]}"
  log_path="${LOG_DIR}/strict_static_matched_seed${SEED}_shard${shard_index}_of${SHARD_COUNT}_gpu${gpu}.log"
...
if [[ "${failed}" -ne 0 ]]; then
  echo "At least one strict static matched shard failed. Check logs under ${LOG_DIR}." >&2
  exit 1
fi
