...
CONFIG="${CONFIG:-configs/generated_stage_budget_output_aware_v1/budget_v3_full.template.yaml}"
DEV_SELECTION="${RUN_ROOT}/08_budget_v3_actual_calibrated/01_dev_seed3/budget_v3_dev_selection.json"
SEED42_SELECTION="${RUN_ROOT}/08_budget_v3_actual_calibrated/02_seed42_confirmation/budget_v3_seed42_selection.json"
...
if [[ -z "${selected_method}" ]]; then
  echo "Budget v3 dev did not pass; stopping before full."
  exit 0
fi
...
if [[ "${seed42_passed}" != "1" ]]; then
  echo "Budget v3 seed42 confirmation did not pass; stopping before full."
  exit 0
fi
...
for seed in ${STAGE_BUDGET_SEEDS:-3}; do
  phase_root="${RUN_ROOT}/08_budget_v3_actual_calibrated/03_frozen_full/seed_${seed}"
  CONFIG="${CONFIG}" RUN_ROOT="${phase_root}" SOURCE_ROOT="${SOURCE_ROOT}" LOG_DIR="${LOG_ROOT}/08_budget_v3_actual_calibrated/03_frozen_full/seed_${seed}" \
