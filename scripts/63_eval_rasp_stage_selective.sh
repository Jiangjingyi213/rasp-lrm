...
VARIANTS="${STAGE_SELECTIVE_VARIANTS:-hidden_pca_linear hidden_pca_nonlinear hidden_uncertainty}"

for seed in ${STAGE_PROBE_SEEDS:-1 2 3}; do
  for variant in ${VARIANTS}; do
    run_dir="${STAGE_PROBE_ROOT}/seed_${seed}/${variant}"
    "${PYTHON}" -m src.main_eval_rasp_stage_selective \
      ...
      --output "${run_dir}/selective_eval.json" \
      --max-setup-false-accept-rate "${STAGE_MAX_SETUP_FALSE_ACCEPT_RATE:-0.10}"
  done
done

"${PYTHON}" scripts/64_summarize_rasp_stage_selective.py --root "${STAGE_PROBE_ROOT}"
