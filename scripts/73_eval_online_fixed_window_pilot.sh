...
export ONLINE_FIXED_WINDOW_GPU_COUNT="${#GPUS[@]}"
CONFIG_DIR="configs/generated_online_fixed_window_pilot"
LOG_DIR="${LOG_DIR:-logs/07_stage_aware/07_online_fixed_window_pilot}"
mkdir -p "${LOG_DIR}"
"${PYTHON}" scripts/72_prepare_online_fixed_window_configs.py
...
for queue in "${!GPUS[@]}"; do
  ...
  nohup bash -lc "
    ...
    while IFS= read -r config; do
      ...
      if [[ -f \"\${summary}\" ]] && ${PYTHON} -c 'import sys; from src.utils.io import read_json, read_yaml; s=read_json(sys.argv[1]); c=read_yaml(sys.argv[2]); r=c[\"runtime_rasp\"]; ok=s.get(\"examples\")==c[\"data\"][\"limit\"] and s.get(\"controller\")==r[\"controller\"] and s.get(\"boundary_tokens\")==r.get(\"boundary_tokens\") and s.get(\"fixed_ratio\")==r.get(\"fixed_ratio\"); raise SystemExit(0 if ok else 1)' \"\${summary}\" \"\${config}\"; then
