...
完整工作流随后已端到端通过。Pilot 覆盖 GSM8K/math_train 各 20 个 eligible problem，共
`332` 个 causal boundary、`1660` 个 action rows；ratio `0.10/0.20/0.30/0.40/0.50` 的 dense-state
flip rate 分别为 `7.23%/10.84%/12.35%/14.76%/18.07%`。Fixed multi-window dev 唯一通过策略为
`r020_c32_m4`，on-policy smoke 的两来源 replay failure 均为 `0`，允许扩大 on-policy bank。

但 on-policy 原始 `candidate_flipped_from_on_policy_dense_control` 是答案变化标签，不等价于
harmful risk：当 dense-control 本身错误时，candidate 改变答案可能是有益纠正。Smoke 中
`ratio=0.10` 的唯一 answer change 就属于此情况。现已显式区分
`candidate_harmful_flip` 和 `candidate_beneficial_correction`；扩大 bank 和 grouped OOF 必须
使用 harmful flip 作为安全风险目标。

旧 smoke 还要求 dense 与 behavior trajectory 都正确，这会把 fixed policy 已经破坏答案的状态
从 on-policy bank 中排除，形成幸存者偏差。扩大前已将准入条件改为只要求 dense trajectory
正确，behavior trajectory 可以错误并作为显式标签保留；训练与 OOF 必须覆盖这些失败状态。
